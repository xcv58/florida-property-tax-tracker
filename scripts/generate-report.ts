...
const latestUpdate = JSON.parse(readFileSync(path.join(root, 'data/update-log.json'), 'utf8'))[0];
...
writeFileSync(path.join(reportsDir, 'latest-update.json'), JSON.stringify(report, null, 2));
writeFileSync(path.join(reportsDir, 'latest-update.md'), markdown);
console.log('Reports written to reports/latest-update.md and reports/latest-update.json.');
