...
const validClaimStatuses = new Set(['confirmed', 'likely', 'disputed', 'outdated', 'retired', 'needs_review']);
...
const bannedLanguage = ['scam', 'sham', 'radical', 'fake', 'guaranteed', 'will pass', 'must pass', 'tax grab', 'giveaway'];
...
function checkSourceIds(owner: string, ids: string[] = []) {
  ids.forEach((id) => {
    if (!sourceIds.has(id)) errors.push(`${owner} references missing source ID: ${id}`);
  });
}

function checkClaimIds(owner: string, ids: string[] = []) {
  ids.forEach((id) => {
    if (!claimIds.has(id)) errors.push(`${owner} references missing claim ID: ${id}`);
  });
}
...
function scanBannedLanguage(owner: string, value: unknown) {
  ...
  bannedLanguage.forEach((phrase) => {
    ...
    if (pattern.test(text)) errors.push(`${owner} contains banned advocacy language: "${phrase}"`);
  });
}
...
sources.forEach((source) => {
  ...
  if (!source.url && !source.archive_path) errors.push(`${source.id} needs either url or archive_path`);
  if (!validSourceTypes.has(source.source_type)) errors.push(`${source.id} has invalid source_type: ${source.source_type}`);
  if (!source.publisher) errors.push(`${source.id} is missing publisher`);
  if (!isIsoDate(source.accessed_date)) errors.push(`${source.id} is missing ISO accessed_date`);
  if (!source.archive_path) warnings.push(`${source.id} has no archived copy`);
  if (source.archive_path) {
    const archiveDiskPath = path.join(root, 'public', source.archive_path.replace(/^\//, ''));
    if (!existsSync(archiveDiskPath)) errors.push(`${source.id} references missing archive file: ${source.archive_path}`);
  }
});
...
claims.forEach((claim) => {
  ...
  if (!validClaimStatuses.has(claim.status)) errors.push(`${claim.id} has invalid status: ${claim.status}`);
  if (claim.status === 'confirmed' && !claim.source_ids?.length) errors.push(`${claim.id} is confirmed without sources`);
  if (!isIsoDate(claim.date)) errors.push(`${claim.id} is missing ISO date`);
  if (!isIsoDate(claim.last_verified)) errors.push(`${claim.id} is missing ISO last_verified`);
});

if (!isIsoDate(status.last_verified)) errors.push('status.last_verified must be an ISO date');
...
status.sentence_claim_ids?.forEach((entry: any) => {
  if (!status[entry.field]?.includes(entry.sentence)) {
    errors.push(`status sentence mapping does not match field text: ${entry.sentence}`);
  }
  checkClaimIds(`status sentence "${entry.sentence}"`, entry.claim_ids);
});
['current_status_summary', 'latest_update', 'expected_next'].forEach((field) => {
  const sentences = String(status[field] ?? '')
    .split(/(?<=[.!?])\s+/)
    .map((sentence) => sentence.trim())
    .filter(Boolean);
  sentences.forEach((sentence) => {
    const hasMapping = status.sentence_claim_ids?.some((entry: any) => entry.field === field && entry.sentence === sentence);
    if (!hasMapping) errors.push(`status.${field} sentence lacks claim mapping: ${sentence}`);
  });
});
...
timeline.forEach((item) => {
  ...
  if (!validTimelineCategories.has(item.category)) errors.push(`${item.id} has invalid category: ${item.category}`);
});

proposals.forEach((proposal) => {
  if (!proposal.source_ids?.length) errors.push(`${proposal.id} has no source_ids`);
  checkSourceIds(`Proposal ${proposal.id}`, proposal.source_ids);
  checkClaimIds(`Proposal ${proposal.id}`, proposal.claim_ids);
  if (!validProposalStatuses.has(proposal.status)) errors.push(`${proposal.id} has invalid status: ${proposal.status}`);
  if (!isIsoDate(proposal.last_action_date)) errors.push(`${proposal.id} is missing ISO last_action_date`);
  if (proposal.status === 'active') {
    const hasOfficialLegislativeSource = proposal.source_ids.some((id: string) => {
      const source = sources.find((candidate) => candidate.id === id);
      return source?.source_type === 'official_legislative';
    });
    if (!hasOfficialLegislativeSource) errors.push(`${proposal.id} is active without official legislative source`);
  }
  if (proposal.status === 'dead' && /currently active/i.test(proposal.current_relevance)) {
...
counties.forEach((county) => {
  ...
  if (!isIsoDate(county.last_verified)) errors.push(`${county.county} is missing ISO last_verified`);
...
if (countiesWithoutEstimates) {
  warnings.push(`${countiesWithoutEstimates} county row(s) have no county impact estimate`);
}

updateLog.forEach((entry) => {
  if (!isIsoDate(entry.date)) errors.push(`Update log entry has invalid date: ${entry.date}`);
